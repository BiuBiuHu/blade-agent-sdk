...
export function analyzeFiles(messages: Message[]): FileReference[] {
  ...
  messages.forEach((msg, index) => {
    ...
    contentFiles.forEach((path) => {
      updateFileReference(fileMap, path, index, false);
    });

    // 从工具调用中提取文件路径
    if (msg.tool_calls && Array.isArray(msg.tool_calls)) {
      msg.tool_calls.forEach((call) => {
        const toolFiles = extractFilePathsFromToolCall(call);
        ...
        toolFiles.forEach((path) => {
          updateFileReference(fileMap, path, index, wasModified);
        });
      });
    }
  });
  ...
  // 返回前 N 个文件
  return sortedFiles.slice(0, MAX_FILES);
}
  ...
  for (const path of filePaths) {
    try {
      ...
      if (totalLines > MAX_LINES_PER_FILE) {
        // 截断文件
        finalContent = lines.slice(0, MAX_LINES_PER_FILE).join('\n');
        finalContent += `\n\n... (truncated ${totalLines - MAX_LINES_PER_FILE} lines)`;
        truncated = true;
 ...
 * @returns 文件路径列表
 */
function extractFilePathsFromContent(content: string): string[] {
  ...
  for (const match of matches) {
    ...
    const pathMatches = extractPathsFromText(block);
  ...
  // 提取普通文本中的文件路径
  const inlineMatches = extractPathsFromText(content);
...
export const FileAnalyzer = { analyzeFiles, readFilesContent };
